...
import { AssemblyGraphError, AssemblyStateError } from '../errors/AssemblyErrors';
...
test('Runtime Assembly Engine Package', async (t) => {
  ...
  await t.test('Runtime Graph Validation detects unreachable cyclic component chain', () => {
    const scan = getMockScan([
      {
        id: 'mod-1',
        type: 'MODULE',
  ...
  await t.test('Successful runtime assembly with deterministic ordering and caching', async () => {
    const scan = getMockScan([
      {
        id: 'mod-2',
        type: 'MODULE',
  ...
  await t.test('Simultaneous parallel assembly requests execution isolation', async () => {
    const scan = getMockScan([
      {
        id: 'mod-1',
        type: 'MODULE',
    ...
    const [res1, res2] = await Promise.all([assembler1.assemble(scan), assembler2.assemble(scan)]);
