...
import { PluginLoadError, PluginStateError, PluginValidationError } from '../errors/PluginErrors';
...
test('Framework Plugin System Package', async (t) => {
  ...
  await t.test('Plugin registration and dependency ordering is deterministic', async () => {
  ...
  await t.test('Dependency resolution rejects cycles and missing dependencies', async () => {
    const builder = new PluginBuilder();
    const manager = new PluginManager(builder.build());
  ...
  await t.test('Plugin initialization and shutdown executes exactly once', async () => {
    const builder = new PluginBuilder();
    const manager = new PluginManager(builder.build());
  ...
  await t.test('Parallel enable()/disable() operations are rejected safely', async () => {
    const builder = new PluginBuilder();
    const manager = new PluginManager(builder.build());
