...
test('Framework Extension Engine Package', async (t) => {
  await t.test('Extension registration and dependency ordering is deterministic', async () => {
  ...
  await t.test('Enable/disable operations update state and diagnostics', async () => {
    const builder = new ExtensionBuilder();
    const manager = new ExtensionManager(builder.build());
