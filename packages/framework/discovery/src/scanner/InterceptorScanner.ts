...
export class InterceptorScanner {
  public scan(
    metadata: MetadataRegistry,
    discoveryRegistry: DiscoveryRegistry,
    discoveryIndex: DiscoveryIndex,
  ): void {
    const list = metadata.resolve(MetadataType.INTERCEPTOR);
    for (const desc of list) {
      if (!desc.parentId) {
        throw new DiscoveryOrphanError(
          `InterceptorScanner: Interceptor "${desc.id}" is missing a parentId mapping.`,
        );
      }
      ...
      if (!parentExists) {
        throw new DiscoveryOrphanError(
          `InterceptorScanner: Interceptor "${desc.id}" has parent ID "${desc.parentId}" but parent is not registered.`,
        );
      }
      ...
      discoveryRegistry.add(MetadataType.INTERCEPTOR, item);
