...
test('Framework Kernel Package', async (t) => {
  const getMockSubsystems = (overrides: Record<string, unknown> = {}): Record<string, unknown> => {
    const list = [
      'Bootstrap',
      'Runtime',
      'HttpServer',
  ...
  await t.test('Successful integration and deterministic startup snapshot', async () => {
